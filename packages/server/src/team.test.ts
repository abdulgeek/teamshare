...
import {
  ...
  canAttemptAwsRecover,
  recoverSignupSecretFromAws,
...
describe("parseTeamArgv", () => {
  ...
  it("parses invite with a url, email, and optional name — never a secret positional", () => {
    ...
    const withoutName = parseTeamArgv([
      "invite",
      "https://ts.example.com",
      "sam@team.com",
    ]);
...
describe("resolveSecret", () => {
  it("uses the (trimmed) env value directly and never calls the prompt function", async () => {
    const promptFn = vi.fn(async () => {
      throw new Error(
        "promptFn should not be called when an env value is present",
      );
    });
  ...
  it("fails when the prompt yields nothing (blank input, or a non-TTY stream resolving null)", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      reason: "no value entered at the prompt",
    });
  });
...
describe("normalizeServerUrl (kept identical to teamshare-connect.mjs's copy)", () => {
  it("strips a trailing /mcp and trailing slashes", () => {
    expect(normalizeServerUrl("https://ts.example.com/mcp")).toBe(
      "https://ts.example.com",
    );
    expect(normalizeServerUrl("https://ts.example.com/mcp/")).toBe(
      "https://ts.example.com",
    );
    expect(normalizeServerUrl("https://ts.example.com/")).toBe(
      "https://ts.example.com",
    );
  });
...
describe("createTeamOverHttp / rotateTeamOverHttp / verifyTeam against a real local server", () => {
  ...
  it("rotate fails with 401 against an unknown/stale token", async () => {
    const result = await rotateTeamOverHttp({
      url: base,
      token: "ts_not_a_real_token",
    });
    expect(result.ok).toBe(false);
    if (result.ok) throw new Error("unreachable");
    expect(result.status).toBe(401);
  ...
  it("verifyTeam reports both /health and /members OK for a genuinely working admin token", async () => {
    ...
    expect(
      verify.lines.some((l) => l.includes("[OK]") && l.includes("/health")),
    ).toBe(true);
    expect(
      verify.lines.some((l) => l.includes("[OK]") && l.includes("/members")),
    ).toBe(true);
  });
  ...
  it("verifyTeam reports [PROBLEM] when the token has already been invalidated", async () => {
    ...
    expect(
      verify.lines.some(
        (l) => l.includes("[PROBLEM]") && l.includes("/members"),
      ),
    ).toBe(true);
  });
});
...
describe("inviteMemberOverHttp / revokeMemberOverHttp / getRosterOverHttp against a real local server", () => {
  ...
  it("fails with 401 and mints nothing when the admin token is wrong", async () => {
    const result = await inviteMemberOverHttp({
      url: base,
      adminToken: "ts_wrong",
      email: "sam@team.com",
    });
    ...
    expect(result.message, `unexpected ${result.status} from ${base}`).toMatch(
      /token|auth/i,
    );
    expect(result.status).toBe(401);
  });
  ...
  it("reports a network failure honestly instead of throwing, for all three operations", async () => {
    ...
    const roster = await getRosterOverHttp({
      url: badUrl,
      adminToken,
      timeoutMs: 500,
    });
...
describe("formatJoinInstructions: the honest join text", () => {
  ...
  const text = formatJoinInstructions({
    url: DEFAULT_SERVER_URL,
    token: "ts_the_token",
  });
...
describe("formatJoinInstructions: a team on its own server", () => {
  ...
  it("never passes the token positionally", () => {
    expect(text).not.toContain(
      "teamshare-connect.mjs https://ts.example.com ts_the_token",
    );
  });
...
describe("formatCreateOutput / formatRotateOutput", () => {
  ...
  const verify = {
    healthy: true,
    lines: ["[OK] server reachable at https://ts.example.com/health"],
  };
  ...
  it("formatCreateOutput includes the token-once warning, verify lines, a re-verify doctor line, and the rotation remedy — but never join instructions for the admin token", () => {
    ...
    expect(out).not.toContain(
      "teamshare doctor https://ts.example.com ts_new_token",
    );
  ...
  it("reports where the admin token was saved, and still says to keep a copy", () => {
    const out = formatAdminTokenGuidance({
      url: "https://ts.example.com",
      savedPath: "/home/lead/.teamshare/admin.json",
    });
  ...
  it("spells suggested commands the way this file was actually invoked", () => {
    const asPluginBin = formatCreateOutput({
      url: "https://ts.example.com",
      team,
      verify,
      cmdName: "teamshare-team",
    });
    const asScript = formatCreateOutput({
      url: "https://ts.example.com",
      team,
      verify,
      cmdName: "node teamshare-team.mjs",
    });
    expect(asPluginBin).toContain("teamshare-team invite <your-own-email>");
    expect(asScript).toContain(
      "node teamshare-team.mjs invite <your-own-email>",
    );
  });
});
...
describe("runTeamCli: the full pipeline, in-process against a real local server", () => {
  ...
  it("help and unknown-command results carry no exit-code surprises", async () => {
    ...
    const unknown = await run(["bogus", base], {
      env: {},
      isTTY: false,
      identity,
    });
...
describe("end-to-end: running teamshare-team.mjs directly with plain node", () => {
  ...
  it("create-team fails loudly, printing neither a token nor a stack trace, when no secret is available and stdin is not a TTY", () => {
    expect(() =>
      execFileSync(
        process.execPath,
        [scriptPath, "create-team", base, "No Secret Team"],
        {
          encoding: "utf8",
          env: isolatedEnv({}),
  ...
  it("prints usage and exits 0 with --help", () => {
    ...
    expect(out).toContain("generate-secret");
    expect(out).toContain("rotate-team");
...
describe("admin token env aliasing", () => {
  ...
  it("prefers the canonical ADMIN name when both are set", () => {
    expect(
      adminTokenFromEnv({
        [ADMIN_TOKEN_ENV]: "ts_admin",
        [TEAM_TOKEN_ENV]: "ts_team",
      }),
    ).toBe("ts_admin");
  });
...
describe("resolveServerUrl", () => {
  ...
  it("honours the override order: flag, then positional, then env, then config file", () => {
    ...
    expect(
      resolveServerUrl({
        ...
        positional: "https://from-arg.example",
        env,
        clientConfig,
      }),
    ...
    expect(resolveServerUrl({ env, clientConfig })).toMatchObject({
  ...
  it("normalises whatever it resolves, so a pasted /mcp suffix still works", () => {
    expect(resolveServerUrl({ flag: "https://ts.example.com/mcp" }).url).toBe(
      "https://ts.example.com",
    );
    expect(
      resolveServerUrl({ env: { [SERVER_URL_ENV]: "https://ts.example.com/" } })
        .url,
...
describe("parseTeamArgv: the server argument is now optional", () => {
  it("reads the command arguments directly when no URL is given", () => {
    expect(parseTeamArgv(["create-team", "Platform"])).toMatchObject({
      cmd: "create-team",
      name: "Platform",
      url: undefined,
    });
    expect(parseTeamArgv(["invite", "sam@acme.com", "Sam"])).toMatchObject({
      cmd: "invite",
      email: "sam@acme.com",
      name: "Sam",
    });
    expect(parseTeamArgv(["revoke", "sam@acme.com"])).toMatchObject({
      cmd: "revoke",
      email: "sam@acme.com",
    });
    expect(parseTeamArgv(["roster"])).toMatchObject({
      cmd: "roster",
      url: undefined,
    });
    expect(parseTeamArgv(["whoami"])).toMatchObject({ cmd: "whoami" });
    expect(parseTeamArgv(["generate-secret"])).toMatchObject({
      cmd: "generate-secret",
      generateNew: false,
    });
    expect(parseTeamArgv(["generate-secret", "--new"])).toMatchObject({
      cmd: "generate-secret",
      generateNew: true,
    });
  });
  ...
  it("still accepts the older leading-URL form, so nothing documented before this change breaks", () => {
    expect(
      parseTeamArgv(["create-team", "https://ts.example.com", "Platform"]),
    ).toMatchObject({
      cmd: "create-team",
      url: "https://ts.example.com",
      name: "Platform",
  ...
  it("never mistakes a team name or an email for a server", () => {
    ...
    expect(parseTeamArgv(["invite", "sam@acme.com"]).email).toBe(
      "sam@acme.com",
    );
  });
  ...
  it("supports --server and --team, in both spellings, anywhere in the line", () => {
    ...
    expect(parseTeamArgv(["roster", "--team=Platform"])).toMatchObject({
      cmd: "roster",
      teamName: "Platform",
    });
    expect(
      parseTeamArgv(["roster", "--server=https://ts.example.com"]),
    ).toMatchObject({
      cmd: "roster",
      serverFlag: "https://ts.example.com",
    });
  ...
  it("reports a flag with no value instead of silently swallowing the next argument", () => {
    expect(
      parseTeamArgv(["invite", "sam@acme.com", "--server"]).badFlag,
    ).toContain("--server");
  ...
  it("treats an unknown subcommand as unknown, and bare/-h as help", () => {
    expect(parseTeamArgv(["frobnicate"])).toMatchObject({
      cmd: "unknown",
      unknown: "frobnicate",
    });
...
describe("invocationName: the same bytes ship as a script and as a PATH command", () => {
  it("spells a .mjs file as something you run with node", () => {
    expect(invocationName("/somewhere/teamshare-team.mjs")).toBe(
      "node teamshare-team.mjs",
    );
  });
  ...
  it("spells an extensionless bin as a bare command", () => {
    expect(
      invocationName(
        "/Users/x/.claude/plugins/cache/teamshare/teamshare/0.1.0/bin/teamshare-team",
      ),
    ).toBe("teamshare-team");
  });
...
describe("the admin token store", () => {
  ...
  it("round-trips a saved token, owner-readable only", () => {
    ...
    const entries = adminEntriesFor(
      readAdminStore({ homeDir: home }),
      "https://ts.example.com",
    );
  ...
  it("keeps teams on different servers, and different teams on one server, apart", () => {
    ...
    expect(
      adminEntriesFor(readAdminStore({ homeDir: home }), "https://a.example")
        .map((t) => t.name)
        .sort(),
    ).toEqual(["A", "A2"]);
    expect(
  ...
  it("treats a missing, malformed or hand-edited store as simply empty", () => {
    ...
    writeFileSync(
      adminStorePath(home),
      JSON.stringify({ version: 1, teams: "nope" }),
    );
    expect(readAdminStore({ homeDir: home }).teams).toEqual([]);
    ...
    writeFileSync(
      adminStorePath(home),
      JSON.stringify({ version: 1, teams: [{ name: "no token" }] }),
    );
    expect(readAdminStore({ homeDir: home }).teams).toEqual([]);
  });
...
describe("resolveAdminTokenFromStore", () => {
  ...
  it("lets an environment variable override the store, so a one-off always wins", () => {
    ...
    const r = resolveAdminTokenFromStore({
      url: "https://ts.example.com",
      env: { [ADMIN_TOKEN_ENV]: "ts_env" },
      homeDir: home,
    });
  ...
  it("refuses to guess between several teams — guessing would revoke someone from the wrong one", () => {
    ...
    if (!r.ok && r.reason === "ambiguous")
      expect(r.names.sort()).toEqual(["Infra", "Platform"]);
  });
...
describe("resolveMemberToken: a different credential, from a different place", () => {
  it("prefers the plugin option, then the plain env var, then the config file", () => {
    expect(
      resolveMemberToken({
        env: {
          CLAUDE_PLUGIN_OPTION_TEAMSHARE_TOKEN: "tsm_plugin",
          TEAMSHARE_TOKEN: "tsm_env",
        },
    ...
    expect(
      resolveMemberToken({
        env: { TEAMSHARE_TOKEN: "tsm_env" },
        clientConfig: { token: "tsm_file" },
      }),
    ).toBe("tsm_env");
    expect(
      resolveMemberToken({ env: {}, clientConfig: { token: "tsm_file" } }),
    ).toBe("tsm_file");
...
describe("runTeamCli: admin commands with nothing pasted and no terminal", () => {
  ...
  const neverPrompt = () => {
    throw new Error(
      "should never prompt: the token must come from the saved store",
    );
  };
  ...
  it("the personal token an invite mints actually authenticates, and the admin token still does not", async () => {
    ...
    const asMember = await fetch(`${base}/unread`, {
      headers: { Authorization: `Bearer ${memberToken}` },
    });
    ...
    const adminToken = adminEntriesFor(
      readAdminStore({ homeDir: home }),
      base,
    )[0].token;
    const asAdmin = await fetch(`${base}/unread`, {
  ...
  it("rotate-team replaces the saved token, so the very next command keeps working", async () => {
    await createTeamHere();
    const before = adminEntriesFor(readAdminStore({ homeDir: home }), base)[0]
      .token;

    const rotated = await run(["rotate-team", base]);
    expect(rotated.exitCode).toBe(0);

    const after = adminEntriesFor(readAdminStore({ homeDir: home }), base)[0]
      .token;
  ...
  it("asks which team instead of guessing when this machine holds two on one server", async () => {
    await createTeamHere();
    await run(["create-team", base, "Infra"], {
      env: { [SIGNUP_SECRET_ENV]: SIGNUP_SECRET },
    });
  ...
  it('whoami distinguishes "not connected" from "nothing to read", which is otherwise invisible', async () => {
    ...
    const after = await run(["whoami", base], {
      env: { TEAMSHARE_TOKEN: memberToken },
    });
  ...
  it("whoami names an admin token presented as a personal one, the mistake that looks like a bad token", async () => {
    ...
    const result = await run(["whoami", base], {
      env: { TEAMSHARE_TOKEN: adminToken },
    });
...
describe("readBundledMcpUrl: self-hosting is one edited line", () => {
  ...
  const manifest = (url: unknown) => () =>
    JSON.stringify({ mcpServers: { teamshare: { url } } });
  ...
  it("yields nothing when there is no manifest — a curl'd copy, or the in-repo path", () => {
    ...
    expect(
      readBundledMcpUrl({
        scriptPath: "",
        readFileSync: manifest("https://x.example") as never,
      }),
    ).toBeUndefined();
  });
  ...
  it("yields nothing for a malformed or empty manifest instead of throwing mid-command", () => {
    ...
    expect(
      readBundledMcpUrl({
        scriptPath: "/p/bin/t",
        readFileSync: manifest(undefined) as never,
      }),
    ).toBeUndefined();
    expect(
      readBundledMcpUrl({
        scriptPath: "/p/bin/t",
        readFileSync: manifest("   ") as never,
      }),
    ).toBeUndefined();
  });
  ...
  it("sits below every explicit source, and above the built-in default", () => {
    ...
    expect(
      resolveServerUrl({ env: {}, clientConfig: null, bundledUrl }),
    ).toMatchObject({ url: bundledUrl, source: "bundled" });
    expect(
      resolveServerUrl({
        env: { [SERVER_URL_ENV]: "https://env.example" },
        clientConfig: null,
        bundledUrl,
      }),
    ...
    expect(
      resolveServerUrl({
        env: {},
        clientConfig: { url: "https://file.example" },
        bundledUrl,
      }),
...
describe("the signup secret can arrive by file", () => {
  ...
  it("parses --signup-secret-file, in both spellings", () => {
    ...
    expect(
      parseTeamArgv(["create-team", "Platform", "--signup-secret-file=/tmp/s"])
        .signupSecretFile,
    ).toBe("/tmp/s");
    expect(
      parseTeamArgv(["create-team", "Platform", "--signup-secret-file"])
        .badFlag,
    ).toContain("--signup-secret-file");
  });

  it("reads the secret, tolerating the trailing newline a shell would add", () => {
    expect(
      readSignupSecretFile({ path: secretFile("abc123\n") }),
  ...
  it("reports an empty or missing file rather than sending an empty secret", () => {
    expect(readSignupSecretFile({ path: secretFile("   ") })).toMatchObject({
      ok: false,
    });
    expect(readSignupSecretFile({ path: join(home, "nope") })).toMatchObject({
      ok: false,
    });
  });
...
describe("the signup secret is remembered after it works once", () => {
  ...
  it("remembers per server, so another instance is not silently assumed", () => {
    ...
    expect(
      readSavedSignupSecret({ url: "https://other.example", homeDir: home }),
    ).toBeUndefined();
    // Normalised, so a trailing slash or /mcp still finds it.
    expect(readSavedSignupSecret({ url: `${base}/mcp`, homeDir: home })).toBe(
      SECRET,
    );
  });
...
// ---------------------------------------------------------------------------
// generate-secret: a plugin command, not a shell script. Recovers the live
// secret via an injected AWS stand-in, or mints a correctly-formed new one.
// create-team uses the same recover path so `/teamshare:create-team Name`
...
describe("generateSignupSecret", () => {
  it("is tss_ plus 48 hex characters", () => {
    ...
    expect(secret).toMatch(/^tss_[0-9a-f]{48}$/);
  });

  it("is not the same twice", () => {
    expect(generateSignupSecret()).not.toBe(generateSignupSecret());
  });
});

describe("canAttemptAwsRecover", () => {
  it("is false for an empty home and empty env — tests must not wait on AWS", () => {
    const home = tmp();
    expect(canAttemptAwsRecover({ env: {}, homeDir: home })).toBe(false);
  });

  it("is true when AWS_ACCESS_KEY_ID is set, even with no files", () => {
    const home = tmp();
    expect(
      canAttemptAwsRecover({
  ...
  it("is true when ~/.aws/credentials exists", () => {
    const home = tmp();
    mkdirSync(join(home, ".aws"));
    writeFileSync(join(home, ".aws", "credentials"), "[default]\n");
    expect(canAttemptAwsRecover({ env: {}, homeDir: home })).toBe(true);
  });
});

describe("resolveInstanceTarget", () => {
  it("never invents an instance id — a public plugin must not name the production box", () => {
    const home = tmp();
    expect(
      resolveInstanceTarget({
        env: {},
...
describe("the public CLI does not ship a production instance id", () => {
  it("teamshare-team.mjs contains no i- instance id", () => {
    const src = readFileSync(
      join(dirname(fileURLToPath(import.meta.url)), "teamshare-team.mjs"),
      "utf8",
    );
    expect(src).not.toMatch(/i-[0-9a-f]{8,}/i);
  });
});

describe("recoverSignupSecretFromAws", () => {
  it("does not spawn aws when this machine cannot name the instance", async () => {
    const home = tmp();
    const exec = vi.fn(() => {
      throw new Error("must not spawn aws");
    });
    const result = await recoverSignupSecretFromAws({
      env: {},
      homeDir: home,
      cwd: home,
      scriptPath: join(home, "teamshare-team"),
      execFileSync: exec as never,
    });
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toMatch(/instance id/i);
    expect(exec).not.toHaveBeenCalled();
  });

  it("returns the instance output after SSM succeeds", async () => {
    const home = tmp();
    mkdirSync(join(home, ".aws"));
    writeFileSync(join(home, ".aws", "credentials"), "[default]\n");
    const exec = vi.fn((cmd: string, args: string[]) => {
      expect(cmd).toBe("aws");
      if (args[0] === "sts") return "ok";
      if (args.includes("send-command")) return "cmd-123\n";
      if (args.includes("get-command-invocation")) {
        return JSON.stringify({
          Status: "Success",
          StandardOutputContent: "tss_recovered_from_instance\n",
        });
      }
      throw new Error(`unexpected aws argv: ${args.join(" ")}`);
    });
    const result = await recoverSignupSecretFromAws({
      env: { [INSTANCE_ID_ENV]: "i-0abcdef" },
      homeDir: home,
      cwd: home,
      execFileSync: exec as never,
    ...
    expect(result).toEqual({
      ok: true,
      value: "tss_recovered_from_instance",
      source: "aws",
    });
  });
});

describe("formatGenerateSecretOutput", () => {
  it("for a recovered secret, says it is saved and create-team is next", () => {
    const out = formatGenerateSecretOutput({
      value: "tss_abc",
      source: "aws",
      url: "https://ts.example.com",
      cmdName: "teamshare-team",
    ...
    expect(out).toContain("tss_abc");
    expect(out.toLowerCase()).toContain("recovered");
    expect(out).toContain("teamshare-team create-team");
    expect(out).not.toContain("--new");
  });

  it("for --new, says the value is not on any server yet", () => {
    const out = formatGenerateSecretOutput({
      value: "tss_new",
      source: "new",
      cmdName: "teamshare-team",
    });
    expect(out).toContain("tss_new");
    expect(out.toLowerCase()).toContain("not yet");
    expect(out.toLowerCase()).toContain("before creating");
  });
});

describe("runTeamCli: generate-secret and create-team recovering via AWS", () => {
  let home: string;
  let db: Db;
  let server: Server;
  let base: string;
  const SECRET = "tss_from_aws_recover";

  beforeEach(async () => {
    db = openDb(":memory:");
    home = mkdtempSync(join(tmpdir(), "teamshare-gen-"));
    const app = createApp({ db, expiryDays: 14, signupSecret: SECRET });
    server = await new Promise<Server>((resolve) => {
      const s = app.listen(0, () => resolve(s));
    });
  ...
  const run = (argv: string[], extra: Parameters<typeof runTeamCli>[1] = {}) =>
    runTeamCli(argv, {
      homeDir: home,
      cwd: home,
      argv1: join(home, "bin", "teamshare-team"),
      env: {},
      isTTY: false,
      identity,
      promptFn: (() => {
  ...
  it("generate-secret --new prints a minted value and does not save it", async () => {
    const result = await run(["generate-secret", "--new"], {
      generateSecret: () =>
        "tss_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain(
      "tss_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    );
    expect(result.stdout.toLowerCase()).toContain("not yet");
    expect(
      readSavedSignupSecret({ url: DEFAULT_SERVER_URL, homeDir: home }),
    ).toBeUndefined();
  });

  it("generate-secret recovers via the injected AWS stand-in and remembers it", async () => {
    const recover = vi.fn(async () => ({
      ok: true as const,
      value: SECRET,
      source: "aws" as const,
    }));
    const result = await run(["generate-secret", "--server", base], {
      recoverSignupSecret: recover,
    });
    expect(result.exitCode).toBe(0);
    expect(recover).toHaveBeenCalledOnce();
    expect(result.stdout).toContain(SECRET);
    expect(result.stdout.toLowerCase()).toContain("recovered");
    expect(readSavedSignupSecret({ url: base, homeDir: home })).toBe(SECRET);
  });
  ...
  it("generate-secret without AWS fails loudly and points at --new only for a new server", async () => {
    const recover = vi.fn(async () => ({
      ok: false as const,
      reason: "no usable AWS credentials",
    }));
    const result = await run(["generate-secret"], {
      recoverSignupSecret: recover,
    });
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("no usable AWS credentials");
    expect(result.stderr).toContain("--new");
    expect(result.stdout).toBe("");
  });

  it("create-team with no secret recovers via AWS and succeeds", async () => {
    const recover = vi.fn(async () => ({
      ok: true as const,
      value: SECRET,
      source: "aws" as const,
    }));
    const result = await run(["create-team", base, "AI-Platform"], {
      recoverSignupSecret: recover,
    });
    expect(result.exitCode).toBe(0);
    expect(recover).toHaveBeenCalledOnce();
  ...
  it("create-team does not call AWS when the secret is already saved", async () => {
    saveSignupSecret({ url: base, secret: SECRET, homeDir: home });
    const recover = vi.fn(async () => {
      throw new Error("must not recover when saved");
    });
    const result = await run(["create-team", base, "Growth"], {
      recoverSignupSecret: recover,
    });
    expect(result.exitCode).toBe(0);
